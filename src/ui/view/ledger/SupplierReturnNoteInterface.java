...
// </editor-fold>
// 
public class SupplierReturnNoteInterface extends javax.swing.JInternalFrame implements BatchSelectorInterface {
    //
    ...
    private void initComponents() {
        ...
        itemDataTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "", "Batch Id", "Description", "Quantity", "Cost Price", "Cost Value", "Sales Price", " Sales Value", "Stock Quantity"
            }
        ) {
        ...
        txtGRNNo.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("G#0"))));
        ...
        jPanel2Layout.linkSize(javax.swing.SwingConstants.HORIZONTAL, new java.awt.Component[] {comboLocation, datePicker, txtF19Number});
    ...
    private void recordSRN() {
        ...
        SupplierReturnNote srn;
        try {
            srn = new SupplierReturnNote(
                    txtF19Number.getText(),
                    txtGRNNo.getText(),
                    datePicker.getDate(),
                    SupplierController.getSupplier(txtSupplier.getText()),
    ...
    private void addNewItem() {     
        ...
        d.centerOnDesktop();
        d.disableGRNSelection(txtGRNNo.getText());
        d.setVisible(true);
        /*
    ...
    private void updateInterface() {
        try {
            GoodRecieveNote grn = GoodRecieveNoteController.getGrn(txtGRNNo.getText());
            if (grn == null) {
                // txtGRNNo.setText("");
                txtGRNNo.setBackground(Color.pink);
                logger.error("Invalid GRN number");
            } else {
                txtGRNNo.setBackground(Color.white);
                txtSupplier.setText(grn.getSupplierId());
            }
        } catch (SQLException ex) {
            // txtGRNNo.setText("");
            txtGRNNo.setBackground(Color.pink);
    ...
    private static class TableModelListenerImpl implements TableModelListener {
            ...
            for (int row = 0; row < model.getRowCount(); row++) {
                try {
                    if (e.getColumn() == 1) {
                        ...
                        try {
                            quantity = Double.parseDouble(model.getValueAt(row, 3).toString());
                            stockQuantity = Double.parseDouble(model.getValueAt(row, 8).toString());
                        ...
                        //Auto generated Column
                        result = Double.parseDouble(model.getValueAt(row, 6).toString())
                                * quantity;
        ...
        private void fillProductDetails(int row, String batchId) {
            Product result;
            try {
                ArrayList<Batch> temp = BatchController.getAllBatches(batchId);
                if (temp != null && temp.size() > 0) {
                    String productId = temp.get(0).getProductId();
                    result = ProductController.getProduct(productId);
                    model.setValueAt(result.getProductName(), row, 2);
                } else {
                    logger.error("Cannot find any Batch matching given details.");
                    model.setValueAt("INVALID", row, 2);
                }
            } catch (SQLException ex) {
                logger.error(ex.getMessage());
                model.setValueAt("INVALID", row, 2);
